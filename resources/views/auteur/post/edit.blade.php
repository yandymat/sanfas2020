...
@section('content')
        <div class="container-fluid">
            <form action="{{ route('auteur.post.update',$post->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                    ...
                    <div class="col-lg-4 col-md-12 col-sm-12 col-xs-12">
                        <div class="card">
                            ...
                            <div class="body">
                                ...
                                <div class="form-group form-float">
                                    <div class="form-line {{ $errors->has('pays') ? 'focused error' : '' }}">
                                        ...
                                        <select name="pays[]" id="pays" class="form-control show-tick" data-live-search="true" multiple="true">
                                            @foreach($pays as $pay)
                                                <option 
                                                    @foreach($post->pays as $postPay) 
                                                        {{ $postPay->id == $pay->id ? 'selected' : '' }}
                                                    @endforeach
                                                    value="{{ $pay->id }}">{{ $pay->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    </div>
                                ...
                                <a href="{{ route('auteur.post.index') }}" class="btn btn-danger m-t-15 waves-effect">ANNULER</a>
